...
The output generated by this system must be directed to stadium field volunteers and venue operational staff operating under critical real-time pressures.
...
Be concise (max 3-4 sentences).`;
    ...
    if (useMock || isCooldownActive) {
        if (isCooldownActive) {
            console.warn(`[Local Cooldown Shield] Active 429 lock. Serving playbook fallback locally.`);
        }
    ...
    try {
        ...
        if (isFallbackTrigger) {
            if (error.status === 429 || (error.message && (error.message.includes("429") || error.message.toLowerCase().includes("quota exceeded")))) {
                console.warn(`[Quota Shield] Rate limit (429) intercepted. Locking Gemini API gateway for 30s.`);
                apiCooldownUntil = Date.now() + 30000;
            }
...
7. Be extremely concise. Cite exact values (occupancy %, queue min, water liters).
  `;
    ...
    if (useMock || isCooldownActive) {
        if (isCooldownActive) {
            console.warn(`[Local Cooldown Shield] Active 429 lock. Serving local state parser fallback.`);
        }
    ...
    try {
        ...
        formattedHistory.unshift({
            role: 'user',
            parts: [{ text: `SYSTEM CONTEXT: ${telemetryContext}\nAct as the Copilot for command staff. Note that any output is directed to stadium field volunteers and venue operational staff operating under critical real-time pressures.` }]
        });
        ...
        if (isQuotaError) {
            console.warn(`[Quota Shield] Rate limit (429) intercepted in chat session. Locking Gemini API gateway for 30s.`);
            apiCooldownUntil = Date.now() + 30000;
            return runLocalStateChatParser(userMessage, currentTelemetry);
        }
